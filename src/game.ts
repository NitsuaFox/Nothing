...
export class Game {
  ...
  async tap(x: number, y: number): Promise<void> {
    ...
    if (this.mode === "dead") {
      log("dead tap → waiting point", { score: this.score, depth: this.depth, best: this.best });
      this.openMenu("dead");
      return;
    }
  ...
  private startRun(): void {
    log("run start", { from: this.mode, user: platform.username, hosted: platform.hosted });
    this.depth = 1;
  ...
  private skipSplash(): void {
    ...
    this.skippedSplash = true;
    this.skipFade = this.intro.orbAlpha;
    log("splash skip → menu", {
      t: Number(this.splashT.toFixed(2)),
      phase: this.intro.phase,
      orb: Number(this.intro.orbAlpha.toFixed(2)),
    });
  ...
  private enterMenu(why: string): void {
    ...
    log("menu", {
      ...
      rank: platform.myRank,
    });
  }

  private openMenu(why: string): void {
    this.startRun();
    this.skippedSplash = true;
    this.skipFade = 1;
    this.intro = menuLook();
  ...
  private updateIntro(dt: number): void {
    if (this.mode === "splash") {
      this.splashT += dt;
      this.intro = titleLook(this.splashT);
      this.splashPhase = this.intro.phase;
      logSplashPhase(this.intro.phase, this.splashT);
      if (this.splashT >= SPLASH_END) this.enterMenu("splash-end");
      return;
    }

    if (this.mode === "menu") {
      if (this.skippedSplash && this.skipFade < 1) {
        const prev = this.skipFade;
        this.skipFade = Math.min(1, this.skipFade + dt / SKIP_FADE);
        if (prev < 1 && this.skipFade >= 1) log("menu fade-in done");
        this.intro = {
          phase: "done",
          studioAlpha: 0,
          titleAlpha: 0,
          orbAlpha: this.skipFade,
          menuAlpha: this.skipFade,
          skyAlpha: this.skipFade,
        };
        return;
      }
      this.intro = menuLook();
    ...
    if (this.menuPromptAlpha > 0) {
      this.menuPromptAlpha = Math.max(0, this.menuPromptAlpha - dt / 0.28);
    }
    this.intro = {
      phase: "done",
      studioAlpha: 0,
  ...
  private drawDead(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.fillStyle = "#fff";
    ctx.font = font(700, hud.deadScoreSize);
    strokeText(ctx, String(this.score), this.cx, this.cy + hud.deadScoreDy, hud.strokeScore);
    ctx.font = font(600, hud.deadLine2Size);
    ctx.globalAlpha = 0.85;
    ctx.fillText(this.newBest ? "N E W  B E S T" : `DEPTH ${this.depth}`, this.cx, this.cy + hud.deadLine2Y);
    ctx.globalAlpha = 0.45;
    ctx.font = font(500, hud.deadLine3Size);
    ctx.fillText(`BEST ${this.best}   ·   PEAK ${this.peakCombo}`, this.cx, this.cy + hud.deadLine3Y);
    if (platform.username || platform.myRank) {
      ...
      ctx.fillText(ident, this.cx, this.cy + hud.deadIdentY);
    }

    this.drawDiscoveries(ctx, this.cy + hud.discoverY);
    this.drawBoard(ctx, {
      x: hud.boardX,
      y: hud.boardY,
      align: hud.boardAlign,
      alpha: hud.portrait ? 0.55 : 0.62,
      limit: hud.boardLimit,
      rowH: hud.boardRowH,
    });

    ctx.globalAlpha = 0.4;
    ctx.textAlign = "center";
    ctx.font = font(500, hud.tapSize);
  ...
  private drawDiscoveries(ctx: CanvasRenderingContext2D, y: number): void {
    const hud = this.hud;
    const cols = hud.discoverCols;
    const rows = Math.ceil(DISCOVERY_ORDER.length / cols);
    const gapX =
      cols === 8
        ? Math.min(92 * hud.scale, (hud.frameW - 80 * hud.scale) / Math.max(1, cols - 1))
        : Math.min(84 * hud.scale, (hud.frameW - 48 * hud.scale) / cols);
    const gapY = 18 * hud.scale;
    ctx.font = font(500, hud.discoverSize);
    for (let row = 0; row < rows; row++) {
      const slice = DISCOVERY_ORDER.slice(row * cols, row * cols + cols);
      const rowW = gapX * (slice.length - 1);
      const x0 = hud.cx - rowW / 2;
      slice.forEach((id, i) => {
  ...
  private drawHud(ctx: CanvasRenderingContext2D): void {
    ...
    if (this.mode === "play" && this.started && !this.banging && chrome > 0.01) {
      ctx.textAlign = "left";
    ...
    if (this.combo >= 1 && !this.banging && this.mode === "play" && chrome > 0.01) {
  ...
  private drawMute(ctx: CanvasRenderingContext2D): void {
    ...
    const cy = y + s / 2;
    const u = s / 44;
    const fade = this.mode === "menu" ? this.intro.menuAlpha : 1;
    ctx.save();
